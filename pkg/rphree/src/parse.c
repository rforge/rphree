...
#include "phrqproto.h"
#if !defined(PHREEQC_CLASS)
static char const svnid[] = "$Id: parse.c 3985 2010-02-01 16:48:18Z dlpark $";
 ...
 */
	...
	if (get_elts_in_species(&char_ptr, trxn.token[0].coef) == RLIB_ERROR)
	{
		return (RLIB_ERROR);
	}
/*
 ...
 */
	qsort(elt_list, (size_t) count_elts, (size_t) sizeof(struct elt_list),
		  elt_list_compare);
	if (elt_list_combine() == RLIB_ERROR)
		return (RLIB_ERROR);
/*
 ...
 */
	...
	for (i = 0; i < count_trxn; i++)
	{
		...
		if (get_elts_in_species(&t_ptr, trxn.token[i].coef) == RLIB_ERROR)
		{
			return (RLIB_ERROR);
		}
	}
 ...
 */
			if (get_elt(t_ptr, element, &l) == RLIB_ERROR)
			{
				return (RLIB_ERROR);
			}
 ...
 */
		if (c == '(')
		{
			...
			paren_count++;
			(*t_ptr)++;
			if (get_elts_in_species(t_ptr, coef) == RLIB_ERROR)
			{
				return (RLIB_ERROR);
			}
			if (get_num(t_ptr, &d) == RLIB_ERROR)
 ...
 *   Get new element and subscript
 */
			if (get_secondary(t_ptr, element, &l) == RLIB_ERROR)
			{
				return (RLIB_ERROR);
			}
			elt_list[count_elts].elt = element_store(element);
 ...
 */
		if (c == '(')
		{
			...
			paren_count++;
			(*t_ptr)++;
			if (get_secondary_in_species(t_ptr, coef) == RLIB_ERROR)
			{
				return (RLIB_ERROR);
			}
			if (get_num(t_ptr, &d) == RLIB_ERROR)
 ...
 */
	...
	/* coefficient */
	if (get_coef(&(trxn.token[count_trxn].coef), ptr) == RLIB_ERROR)
	{
		return (RLIB_ERROR);
	}
	/* name and charge */
	if (get_token(ptr, string, &trxn.token[count_trxn].z, &l) == RLIB_ERROR)
	{
		return (RLIB_ERROR);
	}
