...
static char const svnid[] =
	"$Id: isotopes.c 3985 2010-02-01 16:48:18Z dlpark $";
*/
#if !defined(PHREEQC_CLASS)
 ...
 */
	...
	{
		if (isotope_ratio[j]->ratio == RLIB_MISSING)
			continue;
 ...
 */
	...
	{
		if (isotope_alpha[j]->value == RLIB_MISSING)
			continue;
...
/* ---------------------------------------------------------------------- */
{
	...
	for (j = 0; j < count_calculate_value; j++)
	{
		...
		calculate_value[j]->value = RLIB_MISSING;
	}
	...
	for (j = 0; j < count_isotope_ratio; j++)
	{
		 ...
		 */
		if (calculate_value_ptr->value == RLIB_MISSING)
		{
			isotope_ratio_ptr->ratio = RLIB_MISSING;
			isotope_ratio_ptr->converted_ratio = RLIB_MISSING;
		}
		else
		{
	...
	for (j = 0; j < count_isotope_alpha; j++)
	{
		...
		if (calculate_value_ptr->value == RLIB_MISSING)
		{
			isotope_alpha_ptr->value = RLIB_MISSING;
		}
		else
		{
