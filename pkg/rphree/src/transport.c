...
static char const svnid[] =
	"$Id: transport.c 4016 2010-02-06 03:22:13Z charlton $";
 ...
 */
		if (b_c == 1)
		{
			...
			for (j = 1; j <= floor((LDBLE) nmix / 2); j++)
			{
				...
				if (transp_surf)
				{
					if (disp_surf(stagkin_time) == RLIB_ERROR)
 ...
 */
				...
				if (transp_surf)
				{
					if (diff_stag_surf(i) == RLIB_ERROR)
						error_msg("Error in surface transport, stopping.",
								  STOP);
				}
				if (multi_Dflag == TRUE)
				...
				if (surface[n1].count_charge > 1)
				{
					if (sum_surface_comp
						(&surface[n1], 0, &surface[n1], 0, 1, &surface[n1],
						 surface[n1].comps[0].Dw) == RLIB_ERROR)
						return (RLIB_ERROR);
				}
				f1 = 0;
			}
						...
						if (surface[n2].count_charge > 1)
						{
							if (sum_surface_comp
								(&surface[n2], 0, &surface[n2], 0, 1,
								 &surface[n2],
								 surface[n2].comps[0].Dw) == RLIB_ERROR)
								return (RLIB_ERROR);
						}
						f2 = 0;
					}
					...
					if (i1 <= count_cells)
					{
						if (sum_surface_comp
							(&surface[n1], f1, surface_ptr2, k, mixf,
							 &surface[n1],
							 surface_ptr2->comps[k].Dw) == RLIB_ERROR)
							return (RLIB_ERROR);
						f1 = 1;
					}
					if (i2 > 0)
					...
					if (i2 > 0)
					{
						if (sum_surface_comp
							(&surface[n2], f2, surface_ptr1, k, mixf,
							 &surface[n2],
							 surface_ptr1->comps[k].Dw) == RLIB_ERROR)
							return (RLIB_ERROR);
						f2 = 1;
					}
					if (i1 <= count_cells)
 ...
 */
	...
	if (check_surfaces(source1, source2) == RLIB_ERROR)
		return (RLIB_ERROR);
/*
		...
		for (j = 0; j < mix_ptr->count_comps; j++)
		{
					...
					if (surface[n1].count_comps > 1)
					{
						if (sum_surface_comp
							(&surface[n1], 0, &surface[n1], 0, 1,
							 &surface[n1], surface[n1].comps[0].Dw) == RLIB_ERROR)
							return (RLIB_ERROR);
					}
					f1 = 0;
				}
			...
			if (k == temp_count_surface)
			{
				...
				else
				{
					...
					if (surface[n2].count_comps > 1)
					{
						if (sum_surface_comp
							(&surface[n2], 0, &surface[n2], 0, 1,
							 &surface[n2], surface[n2].comps[0].Dw) == RLIB_ERROR)
							return (RLIB_ERROR);
					}
					f2 = 0;
				}
			...
			if (surf2)
			{
				for (k = 0; k < surface_ptr2->count_comps; k++)
				{
					...
					if (sum_surface_comp
						(&surface[n1], f1, surface_ptr2, k, mixf,
						 &surface[n1], surface_ptr2->comps[k].Dw) == RLIB_ERROR)
						return (RLIB_ERROR);
					f1 = 1;

					if (sum_surface_comp
						(&surface[n2], f2, surface_ptr2, k, -mixf,
						 &surface[n2], surface_ptr2->comps[k].Dw) == RLIB_ERROR)
						return (RLIB_ERROR);
				}
			}
			...
			if (surf1)
			{
				for (k = 0; k < surface_ptr1->count_comps; k++)
				{
					...
					if (sum_surface_comp
						(&surface[n2], f2, surface_ptr1, k, mixf,
						 &surface[n2], surface_ptr1->comps[k].Dw) == RLIB_ERROR)
						return (RLIB_ERROR);
					f2 = 1;

					if (sum_surface_comp
						(&surface[n1], f1, surface_ptr1, k, -mixf,
						 &surface[n1], surface_ptr1->comps[k].Dw) == RLIB_ERROR)
						return (RLIB_ERROR);
				}
			}
...
/* ---------------------------------------------------------------------- */
{
	...
	if ((surf_ptr = surface_bsearch(cell, &n)) == RLIB_ERROR)
		return (OK);
