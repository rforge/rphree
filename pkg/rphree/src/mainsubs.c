...
static char const svnid[] =
	"$Id: mainsubs.c 3985 2010-02-01 16:48:18Z dlpark $";
 ...
 */
	...
	/* SRC pr.status = FALSE; */
#endif
	// MDL: Rphree can choose wheter or not print results on file
 ...
 */
	...
	for (n = 0; n < count_solution; n++)
	{
		...
		if (solution[n] != NULL && solution[n]->new_def == TRUE)
		{
			...
			set(TRUE);
			converge = model();
			if (converge == RLIB_ERROR && diagonal_scale == FALSE)
			{
				diagonal_scale = TRUE;
				set(TRUE);
				converge = model();
			...
			print_all();
			/* free_model_allocs(); */
			if (converge == RLIB_ERROR || converge1 == RLIB_ERROR)
			{
 ...
 */
  ...
  // MDL:  Rprintf("xsolution_save: n_user= %d, pointer=%p\n",n_user,solution_ptr);
  ...
  // MDL: Rprintf("xsolution_save: solution_pe_x = %g\n",solution_pe_x);
  ...
  for (i = 0; i < count_master; i++)
      ...
      if (master[i]->in != FALSE)
	{
	  ...
	  solution_ptr->master_activity[count_master_activity++].la =
	    master[i]->s->la;
	  // MDL: Rprintf("xsolution_save: master_activity [%s] = %g\n",master[i]->elt->name,master[i]->s->la);
	...
	master[i]->elt->name;
      // MDL:    Rprintf("xsolution_save: master[%i]->elt->name = %s\n",i,solution_ptr->totals[count_mass_balance].description);      

      solution_ptr->totals[count_mass_balance].input_conc =
	master[i]->total;
      solution_ptr->totals[count_mass_balance].moles = master[i]->total;
      // MDL: Rprintf("xsolution_save: master[%i]->total = %g\n",i,solution_ptr->totals[count_mass_balance].moles);
  ...
  if (count_solution > 1
    ...
    {  
      // MDL:  Rprintf("xsolution_save:: solution sort()\n");
 ...
 */
	...
	if (errors != 0)
	{    
	  Rprintf(":: run_simulation setjmp errors %i\n",errors); // MDL
	  return errors;
	}
...
// MDL: added simulation<2 because this is the only case for this function; 
//      without it, problems with punch!
	for (simulation = 1; simulation<2; simulation++)
	  {
		...
		output_msg(OUTPUT_GUI_RLIB_ERROR, "\nSimulation %d\n", simulation);
...
/* ---------------------------------------------------------------------- */
SEXP
R_run_simulations (PFN_READ_CALLBACK pfn, void *cookie)
/* ---------------------------------------------------------------------- */
{
  int errors;
  char token[MAX_LENGTH];
 ...
 */
  ...
  if (errors != 0)
  {
    Rprintf(":: R_run_simulation setjmp errors = %i\n",errors);
    PROTECT(Rtmperr = allocVector(INTSXP, 1));
    INTEGER(Rtmperr)[0] = errors;
    UNPROTECT(1);
    return Rtmperr;
  }

  SEXP  Routsim, Rtmpsim;
  // MDL: alloc the total list
  PROTECT(Routsim = allocVector(VECSXP, Rnsim));
 ...
 */
  // MDL: added condition 
  for (simulation = 1; simulation <= Rnsim; simulation++)
  { 

#ifdef RphreeDEBUG   // MDL
    Rprintf("---> : deb ::  simulation      %i!\n",simulation); // MDL
#endif
#ifdef PHREEQ98
    AddSeries = !connect_simulations;
#endif
...
#ifdef RphreeDEBUG // MDL
      Rprintf(": deb ::  after copy entities... \n"); // MDL
#endif
 ...
 */
    ...
    Rtmpsim = R_do_out();
#ifdef RphreeDEBUG  // MDL
    Rprintf(": deb :: mainsub: Rtmpsim ok \n"); // MDL
#endif
    SET_VECTOR_ELT(Routsim, simulation-1, Rtmpsim);
  }
  UNPROTECT(1);
  return(Routsim);
}
