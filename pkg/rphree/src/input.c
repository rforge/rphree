...
static char const svnid[] = "$Id: input.c 3873 2009-12-08 00:56:50Z dlpark $";
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (RCountLine < Rinput_length)
  {
    ...
    // MDL: old buggy implementation
    //    line = Rbuff[RCountLine];
    //    Rprintf("Line[%i/%i]: %s \n", RCountLine, Rinput_length, line);
    //    line_save=line;   
    //    RCountLine++;

    // MDL: check if line is big enough to receive the copy, if not realloc
    len = strlen(Rbuff[RCountLine]);
    if (len > max_line)
      {
#ifdef RphreeDEBUG // MDL
	Rprintf(": deb :: reallocating line, line_save from %i to %i", max_line, (int) len+2);  // MDL
#endif
	max_line = len+2;
	line_save =
    ...
    // MDL: copy the strings (cause the program modifies it)
    strcpy(line,Rbuff[RCountLine]);
    line[len] = '\0';
    ...
    {
      next_keyword = 0;
      // MDL: Rprintf("RCountLine=%i, returning EOF \n", RCountLine);
      return (EOF);
    }
...
/* ---------------------------------------------------------------------- */
{
  ...
  char token[MAX_LENGTH];
#ifdef RphreeDEBUG // MDL
  Rprintf(": deb :: reading database line %i\n",RCountLine_db);
#endif

  return_value = EMPTY;
  if (RCountLine_db < Rdb_length)
  {
    /*
     *   Get line, check for eof
     */
    ...
    // MDL: check if line is big enough to receive the copy, if not realloc
    len = strlen(Rdb_buff[RCountLine_db]);
    if (len > max_line)
      {
#ifdef RphreeDEBUG // MDL
	Rprintf(": deb :: reallocating line, line_save from %i to %i", max_line, (int) len+2); // MDL
    ...
    // MDL: copy the strings (cause the program modifies it)
    strcpy(line,Rdb_buff[RCountLine_db]);
    line[len] = '\0';
    strcpy(line_save,line);
    line_save[len] = '\0';
    return_value = OK;
  ...
  else  
    {
      next_keyword = 0;
#ifdef RphreeDEBUG // MDL
      Rprintf("RCountLine_db=%i, returning EOF \n", RCountLine_db); // MDL
#endif
      return (EOF);
    }
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (s_read_callback.callback == NULL)  {
    // MDL: Rprintf("check_line read_callback needed!\n");
    return EOF;
  }
  if (reading_database()) {
    print = FALSE;
    ...
    /* 			    print); */
    // MDL: reading the input from the buffer
    i= Rinp_get_line_db();        
    RCountLine_db++;
    return(i);
  } 
...
/* Check keyword */
	...
	// MDL:
	if (!reading_database ())
	  // Rprintf("*** get_line: %i, state=%i, line_save=%s \n",i,state,line_save);
	  check_line_return = i;
	return (i);
}
 ...
 */
	...
	// MDL: if (!reading_database())
	// MDL:   Rprintf("*** return value: %i\n",return_value);
