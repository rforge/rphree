...
#if !defined(PHREEQC_CLASS)
static char const svnid[] = "$Id: prep.c 4065 2010-02-17 16:06:38Z dlpark $";
 ...
 */
		...
		if (tot_ptr->gfw <= 0.0)
		{
			if (tot_ptr->as != NULL)
			{
				...
				if (compute_gfw(tot_ptr->as, &(tot_ptr->gfw)) == RLIB_ERROR)
				{
 ...
 */
	...
	special = TRUE;
	for (token_ptr = spec->rxn_s->token + 1; token_ptr->s != NULL;
		 token_ptr++)
	{
 ...
 */
	...
	for (i = 1; i < count_trxn; i++)
	{
		...
		{
			ptr = trxn.token[i].s->secondary->elt->name;
			if (get_secondary_in_species(&ptr, trxn.token[i].coef) == RLIB_ERROR)
			{
				input_error++;
