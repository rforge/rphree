...
#include "phrqproto.h"
#if !defined(PHREEQC_CLASS)
static char const svnid[] = "$Id: tidy.c 4114 2010-02-23 18:36:09Z dlpark $";
...
/* ---------------------------------------------------------------------- */
{
	...
	for (i = 0; i < logk_ptr->count_add_logk; i++)
	{
		...
		if (next_logk_ptr->done == FALSE)
		{
			...
			if (add_logks(next_logk_ptr, repeats + 1) == RLIB_ERROR)
			{
				return (RLIB_ERROR);
			}
		}
 ...
 */
				if (a0 != 0.0 || a1 != 0)
				{
					...
					/* component 1 */
					dnb =
						-2 * a0 * xb * xc2 - 8 * a1 * xb2 * xc2 +
						6 * a1 * xb * xc2 - 4 * a1 * xc * xb4 -
						8 * a1 * xb3 * xc2 - 4 * a1 * xb2 * xc3 -
						2 * a0 * xc * xb2 - 8 * a1 * xc * xb3 +
						6 * a1 * xc * xb2 + 1;
					...
					/* component 2 */
					dnb =
						2 * a0 * xb * xc2 + 2 * a0 * xc3 +
						8 * a1 * xb2 * xc2 + 8 * a1 * xb * xc3 -
						2 * a1 * xb * xc2 - 6 * a1 * xc3;
 ...
 */
	if (check_species_input() == RLIB_ERROR)
	{
		error_msg("Calculations terminating due to input errors.", STOP);
	}
/*
 ...
 */
	...
	for (i = 0; i < count_master; i++)
	{
		...
		{
			if (compute_gfw(master[i]->gfw_formula, &master[i]->gfw) == RLIB_ERROR)
			{
				input_error++;
				sprintf(error_string,
