...
static char const svnid[] =
	"$Id: inverse.c 3985 2010-02-01 16:48:18Z dlpark $";
 ...
 */
	...
	if (check_solns(inv_ptr) == RLIB_ERROR)
	{
 ...
 */
				if (solve_with_mask(inv_ptr, current_bits) == RLIB_ERROR)
				{
 ...
 */
		...
		if (solve_with_mask(inv_ptr, minimal_bits) == RLIB_ERROR)
		{
 ...
 */
	...
	for (i = row_mb; i < row_epsilon; i++)
	{
		...
		{
			output_msg(OUTPUT_MESSAGE,
					   "\tRLIB_ERROR: equality not satisfied for %s, %e.\n",
					   row_name[i],
 ...
 */
	for (i = row_epsilon; i < count_rows; i++)
	{
		...
		{
			output_msg(OUTPUT_MESSAGE,
					   "\tRLIB_ERROR: inequality not satisfied for %s, %e\n",
					   row_name[i],
				   (double) (sum - array[(i * max_column_count) + count_unknowns]));
		}
	}
 ...
 */
	for (i = 0; i < count_unknowns; i++)
	{
		...
		{
			output_msg(OUTPUT_MESSAGE,
					   "\tRLIB_ERROR: Dissolution/precipitation constraint not satisfied for column %d, %s, %e.\n",
