...
listtokens(FILE * f, tokenrec * buf)
{
	...
	while (buf != NULL)
	{
		...
		case tokpercent_error:
			output_msg(OUTPUT_BASIC, "PERCENT_RLIB_ERROR");
			break;
...
{  
  ...
  // MDL: Rpunch_vec saves the punched values. There is a pointer 
  //    with global scope setted to it at the end of the function.
  LDBLE Rpunch_vec[Rpunch_dim];
  ...
  while (!iseos(LINK))
	...
	{
	  /*      fputs(n.UU.sval, stdout); */
	  printf("n.stringval==TRUE\n"); // MDL: I leave this message here in case of errors
	  if (punch.high_precision == FALSE)
	    {
      ...
      else if (punch.high_precision == FALSE)
	{
	  ...
	  if (R_punch == TRUE)
	    {
	      Rpunch_vec[n_user_punch_index] = n.UU.val;
	    }
	}
      else
	{
  ...
  // MDL: global pointer to this vector 
  Rpunch_vec_point = &Rpunch_vec[0];
}
