...
static char const svnid[] = "$Id: read.c 4115 2010-02-23 19:28:31Z dlpark $";
 ...
 */
					   ...
					   TRUE)) != KEYWORD)
	{
	  ...
	  // MDL: Rprintf ("check_line su line[%i] ha ritornato (%i)\n", RCountLine,i);
	  // MDL: Rprintf ("next_keyword= %i\n", next_keyword);

	  if (i == EOF) { // MDL
	    return (EOF);  
	    // MDL: Rprintf("EOF, esco\n");
	  }
...
*/
	for (;;)
	{
		...
		// MDL: Rprintf("db in mem: %i; next_keyword=%i\n", database_in_memory, next_keyword);

		switch (next_keyword)
		{
		case -1:				/* Have not read line with keyword */
		  // MDL:: TODO comment out the next 2 lines?
			sprintf(error_string,
					"Unknown input, no keyword has been specified.");
			warning_msg(error_string);
			// MDL: Rprintf ("Unknown input, no keyword has been specified.\n");
			while ((j =
					check_line("No keyword", FALSE, TRUE, TRUE,
							   TRUE)) != KEYWORD && j != EOF)
			{
			  // MDL TODO comment out next line
 ...
 */
			s_ptr = NULL;
			if (parse_eq(line, &next_elt, association) == RLIB_ERROR)
			{
				parse_error++;
 ...
 */
	ptr = line;
	// MDL: Rprintf("Read_solution line: %s\n",line);
	read_number_description(ptr, &n_user, &n_user_end, &description);
/*
 *   Malloc space for solution data
 */
 ...
 */
	...
	for (;;)
	{
		...
		case 5:				/* redox */
			...
			break;
		case 6:				/* ph */
			if (read_conc(n, count_mass_balance, line) == RLIB_ERROR)
			{
				input_error++;
				break;
			}
 ...
 */
	...
	for (;;)
	{
		...
		// MDL: Rprintf("databse %i, species %i\n ", database_in_memory, opt);
		switch (opt)
		{
		case OPTION_EOF:		/* end of file */
			return_value = EOF;
 ...
 *	 OPTION
 *	 EOF
 *	 RLIB_ERROR if any errors reading LDBLEs
 */

	if (read_line_doubles(next_char, d, count_d, count_alloc) == RLIB_ERROR)
	{
		return (RLIB_ERROR);
	}
	for (;;)
