...
static char const svnid[] = "$Id: print.c 4006 2010-02-04 19:58:29Z dlpark $";
...
// MDL: static int punch_user_punch(void);
 ...
 */
	...
	// MDL: punch_user_punch
	//  Rprintf("called punch_punch\n");
	punch_user_punch();
/*
 ...
 */
	...
	output_msg(OUTPUT_MESSAGE, "\t%-15s%12s%15s   %-15s%15s\n\n", // MDL increased precision for the written output, 12 to 15
	...
	for (i = 0; i < kinetics_ptr->count_comps; i++)
	{
		if (state != TRANSPORT && state != PHAST)
		{
		    output_msg(OUTPUT_MESSAGE, "\t%-15s%15.6e%15.6e", // MDL precision 12.3 -> 15.6
		...
		{
		    output_msg(OUTPUT_MESSAGE, "\t%-15s%15.6e%15.6e", // MDL precision
					   kinetics_ptr->comps[i].rate_name,
					   (double) (kinetics_ptr->comps[i].m -
 ...
 */
		...
		/* MDL:    // Rprintf ("sat_ind: name=%s, si=%7.2f, iap=%8.2f, lk=%8.2f, formula= %s\n", */
		/* 		phases[i]->name, (double) si, (double) iap, */
		/* 		(double) phases[i]->lk, phases[i]->formula); */
		si1 = si;
#endif
		...
		output_msg(OUTPUT_MESSAGE, "\t%-15s%7.2f%8.2f%8.2f  %s\n",
				   ...
				   phases[i]->formula);
		/* MDL:    // Rprintf ("sat_ind2: name=%s, si=%7.2f, iap=%8.2f, lk=%8.2f, formula= %s\n", */
		/* 		phases[i]->name, (double) si, (double) iap, */
		/* 		(double) phases[i]->lk, phases[i]->formula); */
 ...
 */
		...
		if (state != TRANSPORT && state != PHAST)
		{
		        sprintf(token, " %14.6e %14.6e %14.6e", // MDL precision, 11.3 to 14.6
		...
		{
		         sprintf(token, " %14.6e %14.6e %14.6e",// MDL precision
			    //  sprintf(token, " %11.3e %11.3e %11.3e",
 ...
 */
	...
	pure_water = TRUE;
	output_msg(OUTPUT_MESSAGE, "\t%-15s%15s%15s\n\n", // MDL: increased spacing for precision 
		   "Elements", "Molality", "Moles");
	for (i = 0; i < count_unknowns; i++)
	{
		if (x[i] == alkalinity_unknown)
		{
		    output_msg(OUTPUT_MESSAGE, "\t%-15s%15.6e%15.6e\n", // MDL precision, 12.3 -> 15.6
		...
		{
		    output_msg(OUTPUT_MESSAGE, "\t%-15s%15.6e%15.6e",// MDL increased precision
			       x[i]->description,
 ...
 *   pH
 */
	output_msg(OUTPUT_MESSAGE, "%45s%10.6f    ", "pH  = ", // MDL : increased precision
		   (double) (-(s_hplus->la)));
	if (ph_unknown == NULL)
	{
		output_msg(OUTPUT_MESSAGE, "\n");
 ...
 *    pe
 */
	output_msg(OUTPUT_MESSAGE, "%45s%10.6f    ", "pe  = ", // MDL: increased precision
		   (double) (-(s_eminus->la)));
	if (pe_unknown == NULL)
	{
		output_msg(OUTPUT_MESSAGE, "\n");
 ...
 */
        // MDL: flag to be sure that we should punch
        if (R_do_punch == FALSE)
	  return(OK);
	...
	// MDL: Set the flag back to FALSE
	R_do_punch = FALSE;
...
fpunchf_user(int user_index, const char *format, ...)
{
	...
	Rprintf(": deb :: fpunchf_user called\n"); // MDL
#endif
