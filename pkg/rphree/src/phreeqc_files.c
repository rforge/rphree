...
static FILE *error_file = NULL;	/* OUTPUT_RLIB_ERROR */
...
static char const svnid[] =
	"$Id: phreeqc_files.c 3873 2009-12-08 00:56:50Z dlpark $";
#endif
#if !defined(PHREEQC_CLASS)
/* ---------------------------------------------------------------------- */
int CLASS_QUALIFIER
...
// MDL: the following block completely commented out
/* 	/\* */
...
/* ---------------------------------------------------------------------- */
{
	...
	case OUTPUT_RLIB_ERROR:
		...
		if (error_file != NULL)
		{
			fprintf(error_file, "RLIB_ERROR: %s\n", err_str);
			if (flush)
				fflush(error_file);
		}
#ifdef PHREEQ98
		sprintf(progress_str, "RLIB_ERROR: %s\n", err_str);
		show_progress(type, progress_str);
#endif /* PHREEQ98 */
		if (output != NULL)
		{
			fprintf(output, "RLIB_ERROR: %s\n", err_str);
#ifdef PHREEQ98
			outputlinenr++;
#endif
			if (flush)
...
/* ---------------------------------------------------------------------- */
{
	switch (type)
	{
	  ...
	  // MDL: modified the whole OUTPUT_PUNCH case
	case OUTPUT_PUNCH:
	  if (R_fileprint == TRUE)
	    {
#ifdef RphreeDEBUG
	      Rprintf(": deb :: called open_handler OUTPUT_PUNCH\n");
#endif
	      if (punch_file != NULL)
		{
		  fclose (punch_file);
